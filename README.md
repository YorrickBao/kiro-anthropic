...
- **手动排序**：每张卡片有 ↑/↓ 按钮，可手动调整账号在管理页的显示顺序；**仅影响展示**，不改变请求分发与轮询（见下节）。顺序持久化于 `accounts.json`，并随号池导出/导入迁移；排序后新登录/导入的账号排在末尾。
...
`/v1/messages` **始终**从账号池动态选号，没有主账号概念（管理页的手动排序**不参与**选号，轮询/亲和始终基于账号创建时间序）：
