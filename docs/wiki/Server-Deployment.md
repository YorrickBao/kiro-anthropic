...
> 这份文档也作为项目 Wiki 的一页维护。发布方法见页尾「如何发布到 Wiki」。
...
GitHub Wiki 是独立的 git 仓库。把本文件推上去：

```bash
git clone https://github.com/YorrickBao/kiro-anthropic.wiki.git
cp docs/wiki/Server-Deployment.md kiro-anthropic.wiki/Server-Deployment.md
cd kiro-anthropic.wiki
git add Server-Deployment.md
git commit -m "docs: server deployment guide"
git push
```

（首次使用 Wiki 需先在仓库网页的 Wiki 标签页点一次「Create the first page」以初始化 wiki 仓库。）
